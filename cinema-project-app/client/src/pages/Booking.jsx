...
const Booking = () => {
      ...
      if (data.data.length > 0) {
        ...
        let todaysMovies = data.data.map((showing) => {
          if (!processedMovies.includes(showing.movie.title)) {
            let timesStandard = data.data.filter((screening) => {
              return (
                screening.movie.title === showing.movie.title &&
                screening.screen === "Standard"
              );
            });
            timesStandard.sort((time1, time2) => {
              return time1.timeRaw - time2.timeRaw;
            });
            let timesFilteredStandard = timesStandard.map((time) => {
              return (
                <Button
                  onClick={() => {
                    setSelectedScreening(time);
            ...
            let timesDeluxe = data.data.filter((screening) => {
              return (
                screening.movie.title === showing.movie.title &&
                screening.screen === "Deluxe"
              );
            });
            timesDeluxe.sort((time1, time2) => {
              return time1.timeRaw - time2.timeRaw;
            });
            let timesFilteredDeluxe = timesDeluxe.map((time) => {
              return (
                <Button
                  onClick={() => {
                    setSelectedScreening(time);
                ...
                <div style={{ display: "flex" }}>
                  ...
                  <div>
                    ...
                    <CardBody>
                      ...
                      <div style={{ display: "flex" }}>
                        <div>
                          <p style={{ margin: "10px 8px 6px 8px" }}>
                            Standard:
                          </p>
        ...
        setCurrentMovieList(
          <div>
            ...
            <p style={{ marginLeft: "200px" }}>
              Movies on {`${new Date(date).toUTCString().slice(0, 16)}`}:{" "}
            </p>
        ...
        setCurrentMovieList(
          <div>
            <p style={{ marginLeft: "200px" }}>
              Sorry, there are no movies on{" "}
              {`${new Date(date).toUTCString().slice(0, 16)}`}, try another date
              from the calender
            </p>
          </div>
        );
  ...
  if (payments) {
    const clearSeats = () => {
      ...
      axios
        .put(
          `http://localhost:3001/showings/update/${selectedScreening._id}`,
          tempScreening
        )
        .then((data) => {
          console.log(data.data);
      ...
      <div style={{ display: "flex" }}>
        <div>
          ...
          <p>
            ...
            <br></br>
            Showing: {bookingDetails.showingTime} {bookingDetails.showingDate}
            <br></br>
            Screen: {bookingDetails.screen}
            <br></br>
            Tickets: {bookingDetails.adults + bookingDetails.children}
            <br></br>
    ...
    const calculate = () => {
      return selectedScreening.screen === "Deluxe"
        ? (
            bookingDetails.adults * 10.99 +
            bookingDetails.children * 4.99
          ).toFixed(2)
        : (
            bookingDetails.adults * 8.99 +
            bookingDetails.children * 3.99
          ).toFixed(2);
    };
    ...
    const updateBooking = (seat, operation) => {
      ...
      tempObj.showingDate = `${new Date(selectedScreening.date)
        .toUTCString()
        .slice(0, 16)}`;
      let localSeating;
          ...
          <img
            ...
            onMouseEnter={(e) => {
              if (e.target.dataset.clicked != true) {
                e.target.src = seatHover;
              }
            }}
            onMouseLeave={(e) => {
              if (e.target.dataset.clicked != true) {
                e.target.src = seatEmpty;
              }
            }}
            onClick={(e) => {
              e.target.src = seatClicked;
              e.target.dataset.clicked = e.target.dataset.clicked == 0 ? 1 : 0;
              e.target.src = seatHover;
              if (e.target.dataset.clicked == true) {
                updateBooking(e.target.id, "add");
              } else {
                updateBooking(e.target.id, "remove");
              }
            }}
          />
        );
      ...
      <div>
        <div style={{ display: "flex" }}>
          <div style={{ width: "300px" }}>
            <div>
              {selectedScreening.screen}
              <br></br>
              {selectedScreening.movie.title}
              <br></br>
              {selectedScreening.time}{" "}
              {new Date(selectedScreening.date).toUTCString().slice(0, 16)}
              <br></br>
    ...
    return (
      <div style={{ display: "flex", padding: "4em" }}>
        <div>
          ...
          <Input
            type="date"
            style={{ width: "270px", position: "inline-block" }}
            onChange={(e) => {
              console.log(e.target.value);
              moviesGet(e.target.value);
            }}
          />
        </div>
        <div>{currentMovieList}</div>
      </div>
    );
  }
};
