  ...
  const timesDropdown = (timeList) => {
    let timeOptions = timeList.map((time) => {
      ...
      return (
        <DropdownItem
          onClick={() => {
            setCurrentMovie(time);
            setMovieTime(time.time);
            createBookButton();
          }}
        >
          {time.time}
    ...
    return (
      <UncontrolledDropdown style={{ display: "inline-block" }}>
        <DropdownToggle caret>Select a time</DropdownToggle>
        <DropdownMenu>{timeOptions}</DropdownMenu>
      </UncontrolledDropdown>
    );
  };
  ...
  const selectMovie = (title) => {
    ...
    axios.get(`http://localhost:3001/screens/title/e/${title}`).then((data) => {
      ...
      for (let i = 0; i < data.data.length; i++) {
        let filteredShowings = data.data[i].showings.filter((showing) => {
          return showing.movie.title == title ? true : false;
        });
        ...
        filteredShowings.map((showing) => {
          timeList.push(
            (obj = {
              time: `${String(showing.time).slice(0, -2)}:${String(
                showing.time
              ).slice(-2, String(showing.time).length)}`,
              timeNumerical: showing.time,
              screen: data.data[i].name,
        ...
        setTimes(
          timesDropdown(
            timeList.sort((showing, showing2) => {
              return showing.timeNumerical - showing2.timeNumerical;
            })
          )
        );
      }
      ...
      <div style={{ display: "flex" }}>
        <div>
          ...
          <p>
            ...
            <br></br>
            Screen: {bookingDetails.screen.slice(-1)}
            <br></br>
